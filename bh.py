...
part_size = int(particles_number / size)
...
# Table for forces
forces = np.zeros((part_size, dimensions))
...
def update_forces(particles_parameters, forces, tree):
    forces.fill(0)
...
def exchange_particles(my_particles):
    last_message = (rank, my_particles)
    messages = [last_message]
    for i in range(size-1):
        comm.send(last_message, dest=(rank + 1) % size, tag=i)
        last_message = comm.recv(source=(rank - 1) % size, tag=i)
        messages.append(last_message)
    for (index, particles) in messages:
        particles_parameters[index * part_size:(index+1) * part_size] = particles


for it in range(iterations_number):
    track[it] = particles_parameters[:, 0:3]
    my_particles = particles_parameters[rank * part_size:(rank+1) * part_size]
    tree = createTree(particles_parameters)
    update_forces(my_particles, forces, tree)
    update_position_and_velocity(my_particles, forces)
    exchange_particles(my_particles)
